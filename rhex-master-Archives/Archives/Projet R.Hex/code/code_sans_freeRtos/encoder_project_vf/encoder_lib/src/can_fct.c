...
void CAN1_GPIO_Config()
{
	...
	CAN1_GPIO_PORT->AFR[i] |= (GPIO_AF_CAN1 << (CAN1_RX_PIN - 8*i)*4);
	/* Connect CAN1_TX_PIN to CAN (AF9) */
	i = CAN1_TX_PIN >> 0x03;
	CAN1_GPIO_PORT->AFR[i] &= ~(0xF << (CAN1_TX_PIN - 8*i)*4);
	CAN1_GPIO_PORT->AFR[i] |= (GPIO_AF_CAN1 << (CAN1_TX_PIN - 8*i)*4);
}
...
void CAN2_GPIO_Config()
{
	...
	CAN2_GPIO_PORT->AFR[i] |= (GPIO_AF_CAN2 << (CAN2_RX_PIN - 8*i)*4);
	/* Connect CAN2_TX_PIN to CAN (AF9) */
	i = CAN2_TX_PIN >> 0x03;
	CAN2_GPIO_PORT->AFR[i] &= ~(0xF << (CAN2_TX_PIN - 8*i)*4);
	CAN2_GPIO_PORT->AFR[i] |= (GPIO_AF_CAN2 << (CAN2_TX_PIN - 8*i)*4);
}
